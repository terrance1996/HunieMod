...
executable_name="HuniePop";
executable_name_linux_x64="HuniePop.x86_64";
executable_name_linux_x86="HuniePop.x86";
...
doorstop_libs=${PWD}/BepInEx/doorstop_libs;
arch="";
executable_path="";
lib_postfix="";
...
os_type=`uname -s`;
case $os_type in
    Linux*)     if [ -f "${executable_name_linux_x64}" ]; then
                    executable_path=${PWD}/${executable_name_linux_x64}
                else
                    executable_path=${PWD}/${executable_name_linux_x86}
                fi
                lib_postfix="so";;
